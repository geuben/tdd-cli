...
| `Ledger.active_claim`'s cross-host staleness fallback (60-minute age) is untested — no cycle in this plan exercises the `hostname != socket.gethostname()` branch | `src/tddcli/ledger.py:Ledger.active_claim` | The Decisions table calls this branch out explicitly (PIDs meaningless across hosts); it shipped with the same rigor as the same-host path but zero test coverage | Low |
